...
**自動判讀模板誠實化(v3.4 · Honest Templates)**
- 動機:pipeline 只抓 4 個粗顆粒科目(rev/gp/op/np),但舊版判讀模板寫死「廠房折舊、管理行銷、研發市場開拓」等細項描述,對不同產業會誤導(如豐泰 9910 是製鞋業,固定成本不是折舊為主)
- 原則(誠實三段式):
  1. **資料事實**(pipeline 能證明的 pattern · 數字驅動)
  2. **白話解讀**(此 pattern 代表什麼)
  3. **可能原因 + 建議查財報**(用「可能為 A、B 或 C」句式,承認不確定 · 給出具體查什麼財報項目)
- 已重構模板(7 個):
  - M03 負向營運槓桿:移除「廠房折舊與人事」斷言,改列訂單短期波動/一次性費用/費用結構性上升 3 種可能
  - M04 費用失控:移除「管理與行銷支出」斷言,改列推銷/管理/研發膨脹或一次性支出
  - M06 營運槓桿釋放:移除「固定費用稀釋」斷言,改列固定成本稀釋/產能利用率/產品組合/費用管控 4 種可能
  - M07 產品組合優化:移除「高毛利產品比重拉升」斷言,改列產品組合/原料成本/匯率/製程效率
  - M08 強勁定價權:移除「具備轉嫁成本能力」斷言,改列供需/產品升級/成本利多
  - M09 戰略投資期:移除「研發與市場開拓費用」斷言,改列擴張投資(研發/通路/新產能)或一次性費用
  - M10 精實轉型:移除「淘汰低毛利訂單」斷言,改列主動淘汰/組合調整/成本優化/一次性去化
