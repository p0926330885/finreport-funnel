...
- **理由**:
  ...
  - 避開泡泡圖專案使用 FinMind token 的時段(2026-09-22 更正:泡泡圖實際排程為**平日晚上** 18:37 / 20:47 TPE,受 GitHub 延遲實際約 18:40–02:30 執行,週末不跑)
...
**2026-09-22 更正(依兩個 repo 實際 GitHub Actions 紀錄):**

| 專案 | 排程(TPE) | 實際執行時段(含 GitHub 延遲) | FinMind 用量 |
|---|---|---|---|
| 財報漏斗 backfill(v3.6 含 K 線) | 每天 03:30 | 約 05:30–09:40 · 約 2 hr | ~1,000 req/批 · 自我限速 ≤500/hr |
| 財報漏斗 daily-prices | 平日 18:30 / 21:30 | 數十秒 | **0**(證交所 / 櫃買 openapi) |
| 泡泡圖 FinMind_Project | 平日 18:37 / 20:47 | 約 18:40–02:30 | ~316 req/次(158 檔 × 法人 + 股價)· 無限速 · 數分鐘內完成 |

- 兩者實際時段相隔 ≥ 3 小時 · 各自單獨都在 600 req/hr 上限內 → **正常排程下不會撞額度**
- ⚠ 手動觸發財報漏斗請避開**平日 18:30–02:30**(泡泡圖時段);週末整天皆可
- 極端情況(泡泡圖第二班延遲超過 03:30 且漏斗準時開跑)同一小時可能 ~800 req > 600:
  泡泡圖遇 402 會中止並保留快取、下一班自動補(FORCE_REFETCH_DAYS=15);漏斗會 backoff 重試,抓不到的下週批次補
